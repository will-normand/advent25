...
object Day06 extends App {
  ...
  def solveCephalopodProblem(problem: Vector[String]): Long =
    ...
    while (true) {
      val cephlaNumber = inputNumbers
        .map(s => if location >= s.length then "" else s(location).toString())
        .reduce((a, b) => a + b)
      println(
        s"Location $location cephla number $cephlaNumber, inputNumbers $inputNumbers"
      )
